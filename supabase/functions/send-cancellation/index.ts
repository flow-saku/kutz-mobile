...
serve(async (req) => {
  ...
  try {
    const { to, clientName, shopName, serviceName, appointmentDate, appointmentTime, barberEmail, bookingLink } = await req.json();
...
<body style="margin:0;padding:0;background-color:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;padding:40px 0;">
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">
          ...
          <!-- Header -->
          <tr>
            <td style="background:#374151;border-radius:12px 12px 0 0;padding:32px;text-align:center;">
              <p style="margin:0 0 8px;color:#9ca3af;font-size:12px;letter-spacing:2px;text-transform:uppercase;">Kutz</p>
              <h1 style="margin:0;color:#ffffff;font-size:26px;font-weight:700;">Appointment Cancelled</h1>
            </td>
          </tr>
          ...
          <tr>
            <td style="background:#ffffff;padding:32px;border-radius:0 0 12px 12px;">
              <p style="margin:0 0 24px;color:#374151;font-size:16px;line-height:1.6;">
                Hey ${clientName}, your appointment at <strong>${shopName}</strong> has been cancelled.
              </p>

              <!-- Details -->
              <table width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #f3f4f6;">
                <tr>
                  <td style="padding:10px 0;color:#6b7280;font-size:14px;width:40%;">Service</td>
              ...
              <p style="margin:32px 0 0;color:#6b7280;font-size:13px;line-height:1.6;">
                Want to rebook? Open the Kutz app or tap the button above.
              </p>
            </td>
          </tr>
    ...
    const text = `Appointment Cancelled — ${shopName}

Hey ${clientName}, your appointment has been cancelled.

Service: ${serviceName}
Date: ${dateFormatted}
Time: ${timeFormatted}

Want to rebook? Open the Kutz app.
...
© ${new Date().getFullYear()} Kutz`;
    ...
    const payload: any = {
      ...
      subject: `Appointment cancelled at ${shopName}`,
      html,
      text,
    };
    ...
    if (!res.ok) {
      console.error('[send-cancellation] Resend error:', data);
